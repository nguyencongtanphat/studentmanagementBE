...
class semesterReportController {
    static async getSemesterReport(req, res, next){
        try{
            ...
            let results = await sequelize.query("SELECT c.name, c.number, c.idClass, COUNT(p.StudentIdStudent) as passNumber  FROM class c, progress p WHERE p.avgSemScore >= 5.0 and c.idClass = p.ClassIdClass and p.SemesterIdSemester = " + semesterId + " \
